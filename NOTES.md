...
Workspace and agent switching had no keys at all: `switch_workspace`, `previous_workspace`, `next_workspace`, `focus_agent`, `previous_agent` and `next_agent` ship unbound in Herdr. They now follow the same three-layer shape as the rest — `alt` for tabs, `alt+shift` for workspaces, `ctrl+alt` for agents — with the prefix variant always listed alongside, so nothing depends on a chord the terminal might swallow. Cycling uses the bracket pair (`alt+{`/`alt+}` for workspaces, `ctrl+alt+[`/`ctrl+alt+]` for agents) and the pickers gained direct chords (`alt+w` for the workspace picker, `alt+g` for the session navigator), which is the faster route once more than a couple of workspaces exist.

Shifted brackets are written as the literal shifted character (`alt+{`, not `alt+shift+[`). Herdr matches a shifted punctuation key either by the Kitty-protocol alternate codepoint or by the character itself, so the literal form resolves under both the enhanced and the legacy encoding, while the `shift+[` spelling only matches when the terminal reports the base key plus a shift flag. Indexed ranges are the exception — they must be written as digits (`alt+shift+1..9`), and Herdr has an explicit `!@#$%^&*(` mapping for exactly that case.

Herdr is pinned to v0.9.0 and the bundled `herdr-mirror` plugin to v0.4.3, up from v0.7.5 and v0.1.15. The keys and theme schema only gained fields across those releases — `resize_pane_*`, `move_tab_*`, separate light/dark custom colors — so nothing in the config had to be rewritten, and every mirror action id the keybindings reference still exists in the new plugin manifest. The removal of Herdr's single-process `--no-session` mode does not affect this packaging, which always launches the server-backed session.

The mirror build needs `openssh` in `nativeCheckInputs`. Two of its tests (`pane_transport_cleanup_terminates_proxy_command`, `timeout_terminates_proxy_command`) spawn `ssh` with a `ProxyCommand` to assert the proxy process group is killed; without the binary in the sandbox they fail with ENOENT. The hosts they contact are `.invalid`, so the tests stay offline — adding the dependency keeps the coverage instead of skipping the tests.
