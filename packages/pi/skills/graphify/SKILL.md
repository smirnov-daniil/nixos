...
> **No other API keys are read.** When `GEMINI_API_KEY`/`GOOGLE_API_KEY` are unset, semantic extraction falls to the host agent itself — the running session is the LLM. On a host that dispatches subagents (including Pi), dispatch them as written in Part B. On a host that runs the CLI directly in a terminal and cannot dispatch subagents, do not stall: a code-only corpus has no semantic work, so write the empty semantic file (Part B "Fast path") and continue to Part C; for a corpus with docs/papers/images, either set a Gemini key or extract those inline yourself, but in no case prompt for `ANTHROPIC_API_KEY` — that prompt is a misread of this skill.
...
**MANDATORY: You MUST use the subagent tool here. Reading files yourself one-by-one is forbidden - it is 5-10x slower. If you do not use the subagent tool you are doing this wrong.**
...
**Step B2 - Dispatch ALL subagents in one parallel call**

Call the `subagent` tool once with `tasks`, one task per chunk, all using `agent: "graphify-extractor"`. This is the only way they run in parallel. If you submit one task, wait, then submit another, you are doing it sequentially and defeating the purpose.

Concrete shape for 3 chunks:
```
{
  "tasks": [
    {"agent": "graphify-extractor", "task": "extract files 1-15 using the prompt below"},
    {"agent": "graphify-extractor", "task": "extract files 16-30 using the prompt below"},
    {"agent": "graphify-extractor", "task": "extract files 31-45 using the prompt below"}
  ]
}
```
...
- If the file is missing, print a warning: "chunk N missing from disk — re-run with the graphify-extractor agent." Do not silently skip.
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort

If more than half the chunks failed or are missing, stop and tell the user to re-run and ensure the `graphify-extractor` agent is used.

Merge all chunk files into `.graphify_semantic_new.json`. **After the subagent call completes, read each task's real token counts from the tool result's `usage` field and write them back into its chunk JSON before merging** — the chunk JSON itself always has placeholder zeros. Then run:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
from pathlib import Path
