...
`packages/pi/` builds the `pi` coding agent (github.com/earendil-works/pi, packaged via the `pi` flake input `github:lukasl-dev/pi.nix`) as a portable package via `inputs.pi.lib.mkCodingAgent` + `wrapPackage`, shipped through `packages/environment.nix`'s `myTools` like every other tool — not a NixOS module, so it works on the non-NixOS host too. `models.json` configures providers (Anthropic/OpenAI via env API keys, plus a local Ollama entry); in-house TypeScript extensions (`extensions/`) add VCS-aware tool preferences (`prefer-rg.ts`, `prefer-fd.ts`, `prefer-jj.ts`), a lightweight `/plan-mode` toggle, layered memory (`memory.ts`), and a deterministic `/ship` research→plan→implement→review→commit subagent pipeline (`pipeline.ts`, role prompts under `agents/`) built on the vendored upstream `subagent` extension. `skills/` ports the code-review/simplify/verify/security-review/run workflow from Claude Code; `APPEND_SYSTEM.md` (wired via pi's `rules` option) holds always-on system-prompt additions (currently: no inline code comments, explain reasoning in chat/NOTES.md instead). Community `pi` extensions are deliberately not installed at runtime (`pi install`) — that ecosystem is young and spam-flooded with near-identical low-quality packages that would run with tool-exec privilege; everything here is authored in this repo and vendored into the Nix store instead.
