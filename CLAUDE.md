...
`nixos/features/pi/` integrates the `pi` coding agent as a NixOS module — `models.json` for model config and TypeScript extensions (`prefer-rg.ts`, `prefer-fd.ts`) for behavior tweaks.
